<x-layouts.admin title="Edit Kamar">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-ink">Edit Kamar {{ $room->room_number }}</h1>
    ...
    <div class="max-w-xl bg-white border border-gray-200 rounded-lg shadow-sm p-6">
        <form method="POST" action="{{ route('admin.rooms.update', $room) }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                @if ($room->image)
                    <img src="{{ asset('storage/'.$room->image) }}" alt="Foto kamar" class="mt-1 h-24 w-36 object-cover rounded-md" />
                @endif
